...
using VTKToMEDMemRateOfFlow::Grp;
using VTKToMEDMemRateOfFlow::Fam;
...
using namespace VTKToMEDMemRateOfFlow;
...
static MEDCoupling::DataArrayIdType *ConvertVTKArrayToMCArrayInt(vtkDataArray *data)
{
  if(!data)
  ...
  MCAuto<MEDCoupling::DataArrayIdType> ret(MEDCoupling::DataArrayIdType::New());
...
static MEDCoupling::DataArrayDouble *ConvertVTKArrayToMCArrayDoubleForced(vtkDataArray *data)
{
  if(!data)
...
static MEDCouplingUMesh *BuildMeshFromCellArray(vtkCellArray *ca, MEDCoupling::DataArrayDouble *coords, int meshDim, INTERP_KERNEL::NormalizedCellType type)
{
...
static MEDCouplingUMesh *BuildMeshFromCellArrayTriangleStrip(vtkCellArray *ca, MEDCoupling::DataArrayDouble *coords, MCAuto<MEDCoupling::DataArrayIdType>& ids)
{
...
template<class T>
static void AppendToFields(MEDCoupling::TypeOfField tf, MEDCouplingMesh *mesh, const MEDCoupling::DataArrayIdType *n2oPtr, typename MEDFileVTKTraits<T>::MCType *dadPtr, MEDFileFields *fs, double timeStep, int tsId)
{
...
static void AppendMCFieldFrom(MEDCoupling::TypeOfField tf, MEDCouplingMesh *mesh, MEDFileData *mfd, MCAuto<MEDCoupling::DataArray> da, const MEDCoupling::DataArrayIdType *n2oPtr, double timeStep, int tsId)
{
  ...
  MCAuto<MEDCoupling::DataArrayInt> dai(MEDCoupling::DynamicCast<MEDCoupling::DataArray,DataArrayInt>(da));
  MCAuto<MEDCoupling::DataArrayIdType> daId(MEDCoupling::DynamicCast<MEDCoupling::DataArray,DataArrayIdType>(da));
...
static void PutAtLevelDealOrder(MEDFileData *mfd, int meshDimRel, const MicroField& mf, double timeStep, int tsId)
{
  ...
  //const MEDCoupling::DataArrayIdType *o2nPtr(o2n); // todo: unused
  MCAuto<MEDCoupling::DataArrayIdType> n2o;
  mmu->setMeshAtLevel(meshDimRel,mesh);
  const MEDCoupling::DataArrayIdType *n2oPtr(0);
  if(o2n)
    {
...
static std::vector<MCAuto<MEDCoupling::DataArray> > AddPartFields(const MEDCoupling::DataArrayIdType *part, vtkDataSetAttributes *dsa)
{
...
static void ConvertFromUnstructuredGrid(MEDFileData *ret, vtkUnstructuredGrid *ds, const std::vector<int>& context, double timeStep, int tsId)
{
  ...
  MCAuto<MEDCoupling::DataArrayInt> lev(MEDCoupling::DataArrayInt::New()) ;  lev->alloc(nbCells,1);
...
void VTKToMEDMemRateOfFlow::WriteMEDFileFromVTKDataSet(MEDFileData *mfd, vtkDataSet *ds, const std::vector<int>& context, double timeStep, int tsId)
{
  if(!ds || !mfd)
...
void VTKToMEDMemRateOfFlow::WriteMEDFileFromVTKGDS(MEDFileData *mfd, vtkDataObject *input, double timeStep, int tsId)
{
  if(!input || !mfd)
...
void VTKToMEDMemRateOfFlow::PutFamGrpInfoIfAny(MEDFileData *mfd, const std::string& meshName, const std::vector<Grp>& groups, const std::vector<Fam>& fams)
{
  if(!mfd)
    return ;
  if(meshName.empty())
